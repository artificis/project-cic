...
export const RESET_TERMINAL_VALUE_PROMPT_MODE =
  'RESET_TERMINAL_VALUE_PROMPT_MODE';
...
export const setTerminalValuePromptMode = createAction(
  SET_TERMINAL_VALUE_PROMPT_MODE
);
export const resetTerminalValuePromptMode = createAction(
  RESET_TERMINAL_VALUE_PROMPT_MODE
);
...
const reducer = {
  [SPIT_TO_TERMINAL]: (state, { payload }) => ({
    ...
    logs: state.logs
      .concat(payload)
      .slice(-parseInt(process.env.REACT_APP_SCROLLBACK_LINES, 10))
  }),
  ...
  [SET_TERMINAL_VALUE_PROMPT_MODE]: (
    state,
    { payload: { passwordMode, promptLabel, onConfirm } }
  ) => ({
    ...state,
    valuePromptMode: {
      passwordMode,
...
export const terminalValuePromptModeSelector = state =>
  state.terminal.valuePromptMode;
